...
```bash
git clone https://github.com/programmerjide/go-taskify.git
cd go-taskify
```
...
The backend uses environment variables prefixed with `TASKIFY_`. Key variables include:

- `TASKIFY_DATABASE_*` - PostgreSQL connection settings
- `TASKIFY_SERVER_*` - Server configuration
- `TASKIFY_AUTH_*` - Authentication settings
- `TASKIFY_REDIS_*` - Redis connection
- `TASKIFY_EMAIL_*` - Email service configuration
- `TASKIFY_OBSERVABILITY_*` - Monitoring settings

See `apps/backend/.env.example` for a complete list.

## Architecture

This taskify follows clean architecture principles:
